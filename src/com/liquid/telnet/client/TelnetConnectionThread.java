...
public class TelnetConnectionThread implements Runnable {
							...
							case TELNET_WILL:
							{
								if(x+1 < bufferSize)
								{
									...
									switch(buffer[x])
									{
										...
										{
											//Formatted += "(EOR)";
											sendData.push(Messages.getString("TelnetConnectionThread.2") + (char)TELNET_IAC + (char)TELNET_DONT + (char)TELNET_OPTION_EOR); //$NON-NLS-1$
											break;
										}
										default:
										{
							...
							case TELNET_DONT:
							{
								if(x+1 < bufferSize)
								{
									...
									switch(buffer[x])
									{
										...
										{
											//Formatted += "(EOR)";
											sendData.push(Messages.getString("TelnetConnectionThread.3") + (char)TELNET_IAC + (char)TELNET_DONT + (char)TELNET_OPTION_EOR); //$NON-NLS-1$
											break;
										}
										default:
										{
							...
							for(y = x+2; y< bufferSize;y++)
							{
								if(buffer[y] == 'm')
								{
									...
									for(int j=startpos+1;j<=y;j++)
									{
										...
										String code = Messages.getString("TelnetConnectionThread.6"); //$NON-NLS-1$
										if(buffer[j] == ';')
										{
										...
										if(code != Messages.getString("TelnetConnectionThread.7")) //$NON-NLS-1$
	...
	public synchronized void run() {
		...
		try {
			sendMessageText(Messages.getString("TelnetConnectionThread.11"),Color.WHITE,Color.BLACK); //$NON-NLS-1$
			...
			sendMessageText(Messages.getString("TelnetConnectionThread.12"),Color.WHITE,Color.BLACK); //$NON-NLS-1$
            ...
            while(!Thread.currentThread().isInterrupted()){
            	//if(inStream.ready()) {
                	...
                	String tempBuffer = Messages.getString("TelnetConnectionThread.13"); //$NON-NLS-1$
            ...
            inStream.close();
            outStream.close();
    		sendMessageText(Messages.getString("TelnetConnectionThread.14"),Color.WHITE,Color.BLACK); //$NON-NLS-1$
		}
		catch (UnknownHostException e1) {
			sendMessageText(Messages.getString("TelnetConnectionThread.15"),Color.WHITE,Color.BLACK); //$NON-NLS-1$
		} catch (IOException e1) {
			sendMessageText(Messages.getString("TelnetConnectionThread.16"),Color.WHITE,Color.BLACK); //$NON-NLS-1$
			e1.printStackTrace();
		} catch (IllegalStateException e){
    		sendMessageText(Messages.getString("TelnetConnectionThread.17"),Color.WHITE,Color.BLACK); //$NON-NLS-1$
		} catch (Exception e) {
			e.printStackTrace();
    		//sendMessageText("\nException\n",Color.WHITE);
    	}
	...
	private void sendMessageText(String text, int color, int bgcolor)
	{
		...
		updateText.putString(Messages.getString("TelnetConnectionThread.18"), text); //$NON-NLS-1$
		updateText.putInt(Messages.getString("TelnetConnectionThread.19"), color); //$NON-NLS-1$
		updateText.putInt(Messages.getString("TelnetConnectionThread.20"), bgcolor); //$NON-NLS-1$
       	Message m = new Message();
       	m.what = TEXT_UPDATE;
